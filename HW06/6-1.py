...
for row in (GV.values.tolist()):
    a, h, w, s, n, q = row
    games[a, h, w, s, n, q] = NFLmodel.addVar(obj = q, vtype=grb.GRB.BINARY, name = 'GAME(%s)v(%s)slt(%s)wk(%s)on(%s)' % (h, a, s, w, n)) #if game played w this config
    season.append(tuple(row))
...
for t in teams:
    for h in away[t]:
        ...
        myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[t,h,w,s,n,q] for t,h,w,s,n,q in season.select(t,h,'*','*','*','*')) == 1, name=cName)
NFLmodel.update()
...
for w in range(1,18):
    for t in teams:
        ...
        myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[t,h,w,s,n,q] for t,h,w,s,n,q in season.select(t,'*',w,'*','*','*'))+
                                              grb.quicksum(games[a,t,w,s,n,q] for a,t,w,s,n,q in season.select('*',t,w,'*','*','*')) == 1, name=cName)
NFLmodel.update()
...
for w in chain(range(1,4),range(13,18)):
    ...
    myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','BYE',w,'SUNB','BYE','*'))== 0, name=cName)
NFLmodel.update()
...
for w in range(4,13): #could be (1,18) but doesn't matter due to #03
    ...
    myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','BYE',w,'SUNB','BYE','*'))<= 6, name=cName)
NFLmodel.update()
...
for t in byeteams:
    ...
    myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[t,h,w,s,n,q] for t,h,w,s,n,q in season.select(t,'BYE',4,'SUNB','BYE','*'))== 0, name=cName)
NFLmodel.update()
...
for w in range(15,17):
    for time in nightslots:
        ...
        myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','*',w,time,'*','*'))== 1, name=cName)
NFLmodel.update()
...
for w in range(1,18):
    if w != 17:
        ...
        myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','*',w,'SUND','*','*'))== 1, name=cName)
    if w == 17:
        cName = '08a_two_DH_wk_%s' % (w)
        myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','*',w,'SUND','*','*'))== 2, name=cName)
NFLmodel.update()
...
for nk in ['CBS','FOX']:
    for i in range(1,16):
        ...
        myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,wk3window,s,n,q] for a,h,wk3window,s,n,q in season.select('*','*',wk,'SUND',nk,'*'))<= 2, name=cName)
NFLmodel.update()

#08c CBS and FOX double header in week 17
for nk in ['CBS','FOX']:
    cName = '08c_DH_wk17_%s' % (nk)
    myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','*',17,'SUND',nk,'*'))== 1, name=cName)
NFLmodel.update()
...
for w in range(1,18):
    if w != 17:
        ...
        myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','*',w,'SUNN','*','*'))== 1, name=cName)
    if w == 17:
        cName = '09_no_SUNN_wk_%s' % (w)
        myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','*',w,'SUNN','*','*'))== 0, name=cName)
NFLmodel.update()

#10a two MONN in week one
cName = '10a_two_MONN_wk_%s' % (1)
myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','*',1,'MONN','*','*'))== 2, name=cName)
NFLmodel.update()
...
for w in range(2,18):
    if w != 17:
        ...
        myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','*',w,'MONN','*','*'))== 1, name=cName)
    if w == 17:
        cName = '10c_zero_MONN_wk_%s' % (w)
        myConstrs[cName] = NFLmodel.addConstr(grb.quicksum(games[a,h,w,s,n,q] for a,h,w,s,n,q in season.select('*','*',w,'MONN','*','*'))== 0, name=cName)
NFLmodel.update()
...
# NFLmodel.setParam('TimeLimit',60)
